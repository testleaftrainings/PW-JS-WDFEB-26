...
export function updateJSONFile<T>(filePath: string, newData: T): void {
  ...
  /* The function is generic, allowing it to work with any type of data structure.
“The function is designed to accept any type using <T>. When we call it with <accountData>, we are telling the function to treat T as accountData. 
So TypeScript checks that the object we pass matches that structure.” */
...
//If same key exists → later one overrides earlier one.
